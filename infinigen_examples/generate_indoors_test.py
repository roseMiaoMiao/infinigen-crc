...
def compose_indoors(output_folder: Path, scene_seed: int, **overrides):
    ...
    def solve_rooms():
        ...
        builder = FloorPlanStateBuilder(
            factory_seed=scene_seed,
            consgraph=consgraph,  # Pass the constraint graph
        )

        # Add rooms and openings
        builder.create_example_state()

        # Create initial state with proper room relations
        builder._create_room_states()  # Ensure room states are created before solving

        # Solve and get state
        state, _, dimensions = builder.solve()
        solver.state = state  # This is the key line we were missing
